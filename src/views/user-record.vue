		...
		<div id="user">
			...
			<div class="user-info">
				...
				<div class="user-name-gender">
					...
					<i :class="['gender', 'iconfont', gender == '男' ? 'icon-xingbie-nan' : 'icon-xingbie-nv', ]" ></i>
				</div>
...
<script>
import { _getRecordSong1, _addLoveSong, _cancelLoveSong, } from "../network/request.js"
...
export default {
	...
	created() {
		...
		// 得到歌曲的详情信息
		_getRecordSong1({ uid: this.$route.params.uid, currentPage: "1" }).then((response) => {
	...
	methods: {
		...
		handleCurrentChange(val) {
			// 得到歌曲的详情信息
			_getRecordSong1({ uid: this.$route.params.uid, currentPage: val }).then((response) => {
				this.songsData = response.data.data
				console.log(this.songsData)
			})
		},
