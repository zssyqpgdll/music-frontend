<template>
	<div id="users-recommend">
		<el-table :data="usersData" stripe style="width: 100%" :header-cell-style="{ fontWeight: 'normal', border: 'none' }">
			...
			<el-table-column align="center" prop="name" label="姓名" width="150" >
				<!-- 在表格中添加跳转链接 -->
				<template #default="scope">
					<span style="cursor:pointer;" @click="recommendUserDetail(scope.$index)" >
						{{ scope.row.name }}
					</span>
				</template>
			</el-table-column>
			...
			<el-table-column align="center" prop="area" label="所在区域" width="150">
				<template #default="scope">
					{{ scope.row.area }}
				</template>
			</el-table-column>

			<el-table-column align="center" prop="des" label="自我简介" width="200" >
				<template #default="scope">
					{{ scope.row.des }}
				</template>
			</el-table-column>

			<el-table-column align="center" prop="registerTime" label="注册时间" width="160" >
				<template #default="scope">
